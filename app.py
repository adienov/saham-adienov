...
# --- 1. SETTING HALAMAN ---
st.set_page_config(page_title="Noris Trading System V55", layout="wide", initial_sidebar_state="expanded")
...
# --- 4. TAMPILAN UTAMA ---
st.title("📱 Noris Trading System V55")
...
if st.button("🚀 SCAN MINERVINI MARKET"):
    ...
    if not df.empty:
        ...
        with cols[0]: # IHSG
            ...
            st.area_chart((ihsg_data/ihsg_data.iloc[0]-1)*100, height=120, color="#FF4B4B")
            
        for idx, row in enumerate(df.head(3).itertuples()): # Top 3 Stocks
            with cols[idx+1]:
        ...
        st.dataframe(df.style.applymap(lambda x: 'color: #9C27B0; font-weight: bold;' if 'SPIKE' in str(x) else 'color: blue;' if 'BREAKOUT' in str(x) else 'color: green;', subset=['Status']), use_container_width=True, hide_index=True)
