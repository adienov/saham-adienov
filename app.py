        ...
        for idx, row in wl.iterrows():
            ...
            if d:
                ...
                with st.expander(f"📊 {d['Stock']} | Rp {d['Price']:,} | {d['Timing']}"):
                    ...
                    if col_b1.button("🗑️ Hapus", key=f"del_{d['Stock']}"):
                        wl = wl[wl.Stock != d['Stock']]; wl.to_csv(WATCHLIST_FILE, index=False); st.rerun()

# --- TAB 3: PORTO (SIMPLE) ---
with tab3:
    st.subheader("Langkah 3: Peta Portfolio")
    df_p = load_data(DB_FILE, ["Tgl", "Stock", "Entry"])
    if not df_p.empty:
        st.metric("Total Posisi", f"{len(df_p)} Saham")
        st.write("---")
        for idx, row in df_p.iterrows():
            ...
            if cols[3].button("Jual", key=f"s_{idx}"): df_p.drop(idx).to_csv(DB_FILE, index=False); st.rerun()
            st.divider()
