...
# --- 1. SETTING HALAMAN ---
st.set_page_config(page_title="Noris Trading System V29", layout="wide", initial_sidebar_state="expanded")
...
# --- 2. HEADER ---
st.title("📱 Noris Trading System V29")
st.caption("Streamlined Edition: Pure Technical & Smart Fundamental Scanner")
...
with st.expander("📖 KAMUS & CARA BACA (Klik Disini)"):
    ...
    * **🟢 EARLY TREND:** Harga > Garis Merah (Alligator).
    ...
    ### 2. 💎 Valuasi (Smart Mode)
    * Data Fundamental hanya diambil jika saham sudah lolos seleksi Teknikal (BUY).
...
def scan_market(ticker_list, min_val_m, risk_pct, days_back, modal_jt, risk_pct_trade, min_vol_pct, check_fund):
    ...
    for i, ticker in enumerate(ticker_list):
        ...
        try:
            ...
            # 3. AMBIL DATA FUNDAMENTAL (HANYA JIKA SINYAL BUY & USER MINTA)
            pbv = 0.0
            fair_value = 0.0
            val_status = "-"
            ...
            if check_fund and days_back == 0 and is_valid_buy:
                try:
                    time.sleep(0.3) # Jeda Anti-Block
